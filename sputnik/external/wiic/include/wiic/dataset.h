...
public:
	 ...
	 */
	Dataset(const std::string& filename) : loaded(false) { loadDataset(filename); }
	~Dataset();
	...
	inline const Training* trainingAt(const unsigned int i) const { 
		if(i < trainings.size())
			...
			std::cout << "[Error]: requested out of array bound index in dataset." << std::endl;
			return 0;
		}
	}
...
protected:
	 ...
	 * @param[in] addr MAC address of the source device for the dataset
	 */
	void saveHeader(std::ofstream& out, const char* addr) const;
